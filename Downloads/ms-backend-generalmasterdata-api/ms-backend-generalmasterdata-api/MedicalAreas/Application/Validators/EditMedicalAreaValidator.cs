...
{
    public class EditMedicalAreaValidator : Validator
    {
        private readonly MedicalAreaRepository _medicalAreaRepository;

        public EditMedicalAreaValidator(MedicalAreaRepository medicalAreaRepository)
        {
            _medicalAreaRepository = medicalAreaRepository;
        }

        public Notification Validate(EditMedicalAreaRequest request)
        {
            ...
            bool descriptionTakenForEdit = _medicalAreaRepository.DescriptionTakenForEdit(request.Id, request.Description);

            if (descriptionTakenForEdit)
                notification.AddError(CommonStatic.DescriptionMsgErrorDuplicate);

            bool CodeTakenForEdit = _medicalAreaRepository.CodeTakenForEdit(request.Id, request.Code);
